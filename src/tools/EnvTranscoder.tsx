...
export default function EnvTranscoder(){
  ...
  return (
    <VStack gap={4}>
      <Text type="supporting" display="block">Transcode .env ↔ JSON ↔ YAML without exposing secrets. Files stay in the tab — complement with Env Scrubber to mask.</Text>
      <HStack gap={3} wrap="wrap" vAlign="end">
        ...
        {mode==='json2env' && <Switch label="with export" value={withExport} onChange={setWithExport} />}
      ...
      <Banner status="warning" title="Security" description="This transcoder preserves values — it does not mask. Use Env Scrubber before sharing." />
    </VStack>
  );
}
