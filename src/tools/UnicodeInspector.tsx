...
export default function UnicodeInspector(){
  ...
  return (
    <VStack gap={4}>
      <Text type="supporting" display="block">Inspect codepoints, UTF-8 bytes and NFC/NFD normalizations without sending sensitive text.</Text>
      <TextArea label="Text" value={text} onChange={setText} rows={4} hasSpellCheck={false} />
      {moj && <Banner status="warning" title="Encoding suspeito" description={moj} />}
      <VStack gap={2}>
        <Text weight="semibold" display="block">Codepoints ({infos.length}) — first 200</Text>
      ...
      <HStack gap={4} wrap="wrap">
        ...
        <VStack gap={1}><Text weight="semibold" display="block">Normalizations</Text>
          {Object.entries(norms).map(([k,v])=>(
            <Text key={k} type="code" display="block">{k}: {JSON.stringify(v).slice(0,120)}</Text>
          ))}
        </VStack>
      </HStack>
      <Banner status="info" title="Private" description="All via TextEncoder/DOM. For advanced spoofing see Invisible Character Detector and Homograph Detector." />
    </VStack>
  );
}
