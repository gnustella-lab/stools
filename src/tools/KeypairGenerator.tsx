...
export default function KeypairGenerator(){
  ...
  return (
    <VStack gap={4}>
      <Text type="supporting" display="block">Generate RSA/EC key pairs offline via Web Crypto. The private key never leaves this tab — nothing is sent.</Text>
      <HStack gap={3} wrap="wrap" vAlign="end">
        ...
        <button onClick={generate} disabled={busy} style={{padding:'8px 14px', borderRadius:8, background: busy?'#ccc':'#111', color:'#fff', border:'none', cursor: busy?'wait':'pointer'}}>{busy? 'Generating…':'Generate pair'}</button>
      ...
      {pub && (
        <>
          ...
          <VStack gap={2}>
            <HStack gap={2} vAlign="center"><Text weight="semibold" display="block">Private Key (PKCS#8) — keep secret</Text><CopyButton value={priv} /></HStack>
            <CodeBlock code={priv} language="plaintext" width="100%" maxHeight={260} isWrapped hasCopyButton={false}/>
          </VStack>
        </>
      )}
      <Banner status="warning" title="Caution" description="Keys are for dev/test. Do not reuse the same key in production without an HSM. Close the tab to discard." />
    </VStack>
  );
}
